...
public class ViewFirstAdmin {
    ...
    /* 
	 * Feedback labels to show the user where a password error is located.  For this application, the
	 ...
	 * case character, this can be corrected by adding one.  We show this at the end, but in 
	 * reality, the upper case could be anywhere in the password.
	 * 
	 */
	// These error labels change based on the user's input
	...
	private ViewFirstAdmin() {
		...
		text_AdminPassword1.textProperty().addListener((observable, oldValue, newValue) 
				-> {ControllerFirstAdmin.updatePassword(); });
		...
		// Label to display the invalid user name error message
		...
		setupLabelUI(label_PasswordsDoNotMatch, "Arial", 18, width, Pos.CENTER, 0, 320);

		// Establish an error message for when there is no input
		setupLabelUI(noInputFound, "Arial", 14, width, Pos.BASELINE_LEFT, 50, 570);
		noInputFound.setText("Please enter a password.");	// This changes based on input, but we 
		...
		setupLabelUI(label_Requirements, "Arial", 14, width, Pos.BASELINE_LEFT, 50, 360);
		
		setupLabelUI(label_UpperCase, "Arial", 14, width, Pos.BASELINE_LEFT, 50, 390);
		
		setupLabelUI(label_LowerCase, "Arial", 14, width, Pos.BASELINE_LEFT, 50, 420);
		
		setupLabelUI(label_NumericDigit, "Arial", 14, width, Pos.BASELINE_LEFT, 50, 450);
		
		setupLabelUI(label_SpecialChar, "Arial", 14, width, Pos.BASELINE_LEFT, 50, 480);
		
		setupLabelUI(label_LongEnough, "Arial", 14, width, Pos.BASELINE_LEFT, 50, 510);
		
		setupLabelUI(label_ShortEnough, "Arial", 14, width, Pos.BASELINE_LEFT, 50, 540);
		...
		theRootPane.getChildren().addAll(label_ApplicationTitle, label_TitleLine1,
				...
				button_Quit, noInputFound, label_Requirements, label_UpperCase, 
				label_LowerCase, label_NumericDigit, label_SpecialChar, 
				label_LongEnough, label_ShortEnough, label_InvalidUsername);
	}
	...
	static protected void resetAssessments() {
	    label_UpperCase.setText("At least one upper case letter");
	    ...
	    label_NumericDigit.setText("At least one numeric digit");
	    label_NumericDigit.setTextFill(Color.RED);
	    
	    label_SpecialChar.setText("At least one special character");
	    label_SpecialChar.setTextFill(Color.RED);
	    
	    label_LongEnough.setText("At least eight characters");
	    label_LongEnough.setTextFill(Color.RED);
	    
	    label_ShortEnough.setText("Shorter than 15 characters");
